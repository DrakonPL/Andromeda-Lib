...
void Crowd::Resize(unsigned int size) {
	...
	mScales.resize(size, AnimVec3(1, 1, 1));
	mFrames.resize(size);
	mTimes.resize(size);
...
void Crowd::RandomizePositions(std::vector<AnimVec3>& existing, const AnimVec3& min, const AnimVec3& max, float radius) {
	std::vector<AnimVec3> positions;
	unsigned int size = (unsigned int)mCurrentPlayTimes.size();
	positions.reserve(size);
	AnimVec3 delta = max - min;
	...
	while (positions.size() < size) {
		...
		AnimVec3 random((float)rand() / (float)RAND_MAX, (float)rand() / (float)RAND_MAX, (float)rand() / (float)RAND_MAX);
		AnimVec3 point = min + delta * random;
...
/*void Crowd::SetUniforms(AnimShader* shader) {
	Uniform<AnimVec3>::Set(shader->GetUniform("model_pos"), mPositions);
	Uniform<AnimQuat>::Set(shader->GetUniform("model_rot"), mRotations);
	Uniform<AnimVec3>::Set(shader->GetUniform("model_scl"), mScales);
	Uniform<AnimIvec2>::Set(shader->GetUniform("frames"), mFrames);
