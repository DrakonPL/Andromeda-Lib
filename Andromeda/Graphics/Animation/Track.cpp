...
template class Track<AnimVec3, 3>;
template class Track<AnimQuat, 4>;

template class FastTrack<float, 1>;
template class FastTrack<AnimVec3, 3>;
template class FastTrack<AnimQuat, 4>;
...
namespace TrackHelpers {
	...
	inline AnimVec3 Interpolate(const AnimVec3& a, const AnimVec3& b, float t) {
		return lerp(a, b, t);
	}

	inline AnimQuat Interpolate(const AnimQuat& a, const AnimQuat& b, float t) {
		AnimQuat result = mix(a, b, t);
		if (dot(a, b) < 0.0f) {
			result = mix(a, -b, t);
		}
	...
	inline AnimVec3 AdjustHermiteResult(const AnimVec3& v) {
		return v;
	}

	inline AnimQuat AdjustHermiteResult(const AnimQuat& q) {
		return normalized(q);
	}

	inline void Neighborhood(const float& a, float& b) { }
	inline void Neighborhood(const AnimVec3& a, const AnimVec3& b) { }
	inline void Neighborhood(const AnimQuat& a, AnimQuat& b) { 
		if (dot(a, b) < 0.0f) {
			b = b * -1.0f;
		}
	}
...
template<> AnimVec3 Track<AnimVec3, 3>::Cast(float* value) {
	return AnimVec3(value[0], value[1], value[2]);
}

template<> AnimQuat Track<AnimQuat, 4>::Cast(float* value) {
	AnimQuat r = AnimQuat(value[0], value[1], value[2], value[3]);
	return normalized(r);
}
...
template FastTrack<AnimVec3, 3> OptimizeTrack(Track<AnimVec3, 3>& input);
template FastTrack<AnimQuat, 4> OptimizeTrack(Track<AnimQuat, 4>& input);
